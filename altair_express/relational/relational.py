...
def lineplot_new(data=None, *, x=None, y=None,color=None,filters=None,effects=None,width=200,height=200):
  ...
  if color:
    if color not in data.columns:
        ...
        unique = np.unique(data[color])
        # background should be like this

        #chart=chart.encode(alt.Color(legend=None,field=color,scale=alt.Scale(domain=unique,range=['lightgray' for value in unique])))
